            ...
            string action, string Identifier, string specificAction, ILogger log)
        {
            ...
            switch (action)
            {
                case "Find": //check if the user allready sign as a role
                    //MainManager.Instance.myLogger.LogEvent"looking for a user in the DB", LoggingLibrary.LogLevel.Event);
                    try { 
                ...
                case "DELETEAPRODUCT"://the user can see only products that are available and didnt ordered
                    ...
                    //MainManager.Instance.myLogger.LogEvent"DeleteAProduct called", LoggingLibrary.LogLevel.Event);
                    try
                    {
